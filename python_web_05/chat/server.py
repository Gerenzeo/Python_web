    ...
    async def distrubute(self, ws: WebSocketServerProtocol):
        async for message in ws:
            if message.startswith('exchange'):
                data = await show_data(int(str(message.split(' ')[1]).strip()))
                ...
                task_1 = await self.write_log(f'{str(datetime.now())} - [INFO] - {ws.name} using command exchange!')
                task_2 = await self.send_to_clients(f'{ws.name}: {"".join([exc for exc in exchanges])}')
